...
fn splash_screen(
    ...
    // If we are not in StartScreen we need to remove ALL the other UI stuff
...
impl Plugin for MenuPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnEnter(TurnState::StartScreen), splash_screen)
            // setup when on the start screen
            .add_systems(
                Update,
                start_screen_input.run_if(in_state(TurnState::StartScreen)),
            )
